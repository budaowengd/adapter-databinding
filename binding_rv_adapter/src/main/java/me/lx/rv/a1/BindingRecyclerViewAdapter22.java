...
import static me.lx.rv.BuildConfig.DEBUG;

/**
 * A {@link RecyclerView.Adapter} that binds items to layouts using the given {@link XmlItemBinding}.
 * If you give it an {@link ObservableList} it will also updated itself based on changes to that
 * list.
 */
public class BindingRecyclerViewAdapter22<T> extends RecyclerView.Adapter<ViewHolder>  {
    ...
    public void setItemBinding(@NonNull XmlItemBinding<T> itemBinding) {
    ...
    @NonNull
    public XmlItemBinding<T> getItemBinding() {
        if (itemBinding == null) {
    ...
    public int getItemCount() {
        Ls.d("getItemCount()..111...size="+items.size());
        return items == null ? 0 : items.size();
    }
    @Override
    public int getItemViewType(int position) {
        Ls.d("getItemViewType()..222...pos="+position);
        itemBinding.xmlOnItemBind(position, items.get(position));
    ...
    private static class WeakReferenceOnListChangedCallback<T> extends ObservableList.OnListChangedCallback<ObservableList<T>> {
        final WeakReference<BindingRecyclerViewAdapter22<T>> adapterRef;

        WeakReferenceOnListChangedCallback(BindingRecyclerViewAdapter22<T> adapter, ObservableList<T> items) {
            this.adapterRef = AdapterReferenceCollector2.createRef(adapter, items, this);
        }

        @Override
        public void onChanged(ObservableList sender) {
            BindingRecyclerViewAdapter22<T> adapter = adapterRef.get();
            if (adapter == null) {
                return;
            }
        ...
        public void onItemRangeInserted(ObservableList sender, final int positionStart, final int itemCount) {
            ...
            if (DEBUG) {
                Ls.d("onItemRangeInserted()..111..positionStart="+positionStart+"  itemCount="+itemCount);
            }
